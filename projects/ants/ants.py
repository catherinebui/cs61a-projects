...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        n=0
        while start!=hive:
            if random_or_none(start.bees) is not None and self.min_range<=n<=self.max_range:
                return random_or_none(start.bees)
            else:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        bees_in_place=[bee for bee in self.place.bees]
    ...
    def action(self, colony):
        ...
        if len(self.place.bees)>0:
            for i in self.place.bees:
                i.armor=i.armor-self.damage
            self.place.bees =[i for i in self.place.bees if i.armor>0]
        # END Problem 7
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6
        if self.digesting==0:
            beed=random_or_none(self.place.bees)
            if beed !=None:
                self.eat_bee(beed)
...
# END Problem 13
    ...
    def __init__(self):
        ...
        self.armor=1
        self.check=[]
        QueenAnt.instance_check+=1
        self.instance=QueenAnt.instance_check
        # END Problem 13
    ...
    def action(self, colony):
        ...
        if self.instance>1:
            ...
            while start.exit:
                start=start.exit
                if start.ant:
                    if start.ant not in self.check :
                        if start.ant.container and start.ant.ant:
                            start.ant.damage = start.ant.damage * 2
                            if start.ant.ant not in self.check:
                                start.ant.ant.damage = 2 * start.ant.ant.damage
                                self.check = self.check + [start.ant.ant, start.ant]
                        else:
                            start.ant.damage=start.ant.damage*2
                            self.check=self.check+[start.ant]
                    else:
                        if start.ant.container and start.ant.ant:
                            if start.ant.ant not in self.check:
                                start.ant.ant.damage = 2 * start.ant.ant.damage
    ...
    def reduce_armor(self, amount):
        ...
        if self.instance==1:
            ...
            return Insect.reduce_armor(self, amount)
        # END Problem 13
